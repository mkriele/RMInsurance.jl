...
state_orig =
  Float64[RMI.dynstate(𝑡,cap_mkt) for 𝑡 ∈ 1:T]
state_avg_orig =
  Float64[RMI.dynstateavg(𝑡,cap_mkt) for 𝑡 ∈ 1:T]
...
RMI.goingconcern!(l_other, liab_ins.Δgc)
...
for 𝑑 ∈ 1:nrow(df_portfolio)
  for 𝑡 ∈ 1:(T + df_portfolio[𝑑, :t_start])
    @test RMI.tpg(𝑡, cap_mkt.rfr.x, liab_ins.mps[𝑑]) ≈
...
## bi-quotient --------------------------------------------------
@test RMI.bonusrate(1, y_invest[1], liab_ins.mps[1], dyn) ≈
      t_bonus_quota[1]
@test RMI.getyield(1, cap_mkt.rfr) ≈ rfr[1]
@test RMI.getyield(1, cap_mkt.stock) ≈ y_stock[1]

ind_bonus_1 =
  y_stock[1] / (t_bonus_quota[1] + liab_ins.mps[1].rfr_price[1])

@test RMI.getyield(0, cap_mkt.stock) ≈ cap_mkt.stock.yield_0
@test RMI.getyield(0, cap_mkt.rfr) ≈ cap_mkt.rfr.yield_0
...
@test bi_quot_1 ≈ RMI.biquotient(1,
                                   y_invest[1],
                                   cap_mkt, invs,
                                   liab_ins.mps[1],
                                   dyn)
...
δ_sx_one =
  Float64[RMI.δsx(1, cap_mkt, invs, liab_ins.mps[𝑑], dyn)
   for 𝑑 ∈ 1:nrow(df_portfolio)]
...
@test invest_eoy_pre_divid ≈
      Float64[RMI.investpredivid(𝑡, invs, proj) for 𝑡 ∈ 1:T]
...
@test [tp_cost_abs_0; tp_cost_abs] ≈
      Float64[RMI.tpgfixed(𝑡, cap_mkt.rfr.x[1:liab_ins.dur],
                             proj.fixed_cost_gc)
              for 𝑡 ∈ 0:T]
...
rfr_up = RMI.rfrshock(cap_mkt.rfr.x, s2_mkt_int, :spot_up)
rfr_down = RMI.rfrshock(cap_mkt.rfr.x, s2_mkt_int, :spot_down)

@test rfr ≈ RMI.spot2forw(RMI.forw2spot(rfr))
@test rfr_down ≈ RMI.spot2forw(RMI.forw2spot(rfr) .*
                 (1 .+ ds2_mkt_int[:shock][:spot_down][1:T]))
@test rfr_up ≈ RMI.spot2forw(
                   RMI.forw2spot(rfr) .+
